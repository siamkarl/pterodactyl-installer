...
# Function to check if the script is running on Windows via WSL
is_wsl() {
    if grep -q -i "microsoft" /proc/version &>/dev/null; then
        return 0  # True (WSL)
    else
        return 1  # False (Linux)
    fi
}

# If running on Windows via WSL
if is_wsl; then
    echo "Running in Windows (via WSL)."
    ...
    # Ensure the OS is Ubuntu or Debian
